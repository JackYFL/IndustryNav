...
# `python -m nav.scripts.agent.run_benchmark_cell` invocation per point.
#
# Usage:
#   bash shs/agent/run_headless_benchmark.sh <scene_code> [model_id]
#   BASELINE=astar bash shs/agent/run_headless_benchmark.sh scene1
#
...
PY
  ...
  CMD=("$PYTHON_BIN" -m nav.scripts.agent.run_benchmark_cell
       --baseline "$BASELINE"
       --file_name "$CLIENT"
       --scene_id "$SCENE_ID"
