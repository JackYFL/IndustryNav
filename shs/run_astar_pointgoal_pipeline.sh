...
"${PYTHON_BIN}" -m nav.scripts.collect_astar_pointgoal \
  --manifest "${MANIFEST}" \
...
"${PYTHON_BIN}" -m nav.scripts.export_astar_pointgoal_dataset \
  --manifest "${MANIFEST}" \
...
"${PYTHON_BIN}" -m nav.scripts.validate_pointgoal_dataset \
  "${OUTPUT_ROOT}/bc" \
