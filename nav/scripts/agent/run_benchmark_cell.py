...
def parse_args():
    ...
    p.add_argument(
        ...
        type=str,
        default="llm",
        choices=[*BENCHMARK_BASELINES, "dagger"],
        help="Decision baseline: random|llm|bc|astar|dagger.",
    )
    ...
    p.add_argument("--ppo_ckpt", type=str, default="")
    p.add_argument("--ppo_device", type=str, default="auto")
    p.add_argument(
    ...
    p.add_argument(
        ...
        default=0.5,
        help=(
            "Probability of executing the A* expert action during DAgger; "
            "the expert label is recorded at every visited state."
        ),
    )
    p.add_argument(
        "--dagger_seed",
        type=int,
        default=0,
        help="Seed for deterministic DAgger expert intervention sampling.",
    )
...
def action_space_for_baseline(
    ...
    if baseline in {"dagger", "ppo"} or (baseline == "astar" and astar_policy_actions) or (
...
def main():
    ...
    if not 0.0 <= args.dagger_beta <= 1.0:
        raise SystemExit("--dagger_beta must be in [0, 1].")
    try:
    ...
    astar_planner = None
    if args.baseline in {"astar", "dagger"}:
        astar_policy_actions = (
            args.astar_policy_actions or args.baseline == "dagger"
        )
        astar_debug_dir = (
        ...
        logger.info(
            ...
            f"policy_actions={astar_policy_actions} | "
    ...
    if args.baseline == "ppo":
        ...
        logger.info(
            f"PPO agent loaded from {args.ppo_ckpt} | "
            f"device={ppo_controller.device}"
        )
    dagger_rng = np.random.default_rng(args.dagger_seed)
    ...
    dagger_labels_path = os.path.join(args.frame_save_dir, "dagger_labels.csv")
    _dagger_labels_file = (
        open(dagger_labels_path, "w", newline="")
        if args.baseline == "dagger"
        else None
    )
    _dagger_labels_writer = (
        csv.DictWriter(_dagger_labels_file, fieldnames=DAGGER_LABEL_FIELDS)
        if _dagger_labels_file is not None
        else None
    )
    if _dagger_labels_writer is not None:
        _dagger_labels_writer.writeheader()
        _dagger_labels_file.flush()
    ...
    try:
        while True:
            ...
            if restored_pending is not None or (decision_thread is not None and not decision_thread.is_alive()):
                ...
                step_count += 1
                if args.baseline in {"astar", "dagger"} and _astar_paths_file is not None:
                    path_record = {
                ...
                if args.baseline in {"astar", "dagger"} and step_budget_mode == "dynamic":
                ...
                if reached:
                    ...
                    if args.baseline == "random":
                        ...
                        payload = {
                            ...
                            "depth_obs": (
                                np.array(depth_obs_raw, copy=True)
                                if depth_obs_raw is not None else None
                            ),
                            "curr_world_x": float(pos_x),
                            "curr_world_z": float(pos_z),
                            "curr_yaw_deg": float(rot_y),
                        ...
                        if curr_xy is not None and target_xy is not None:
                            payload = {
                                ...
                                "ego_obs": (
                                    np.array(ego_obs, copy=True)
                                    if ego_obs is not None else None
                                ),
                                "depth_obs": (
                                    np.array(depth_obs_raw, copy=True)
                                    if depth_obs_raw is not None else None
            ...
            _action_csv_file.flush()
            if (
                _dagger_labels_writer is not None
                and decision_result.get("dagger_step") == step_count - 1
            ):
                policy_action = decision_result["dagger_policy_action"]
                expert_action = decision_result["dagger_expert_action"]
                _dagger_labels_writer.writerow({
                    "step": step_count,
                    "frame_step": step_count - 1,
                    "policy_action": policy_action,
                    "expert_action": expert_action,
                    "executed_action": chosen_action,
                    "used_expert": bool(
                        decision_result["dagger_used_expert"]
                    ),
                    "disagreement": policy_action != expert_action,
                    "beta": decision_result["dagger_beta"],
                    "expert_plan_status": decision_result.get(
                        "astar_plan_status", "unknown"
                    ),
                })
                _dagger_labels_file.flush()
