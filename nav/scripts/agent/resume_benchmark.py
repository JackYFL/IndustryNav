...
Usage: python -m nav.scripts.agent.resume_benchmark outputs/.../seed0 [--dry-run]
...
from nav.scripts.agent.run_benchmark_grid import free_tcp_port
...
def resume_command(folder, config, base_port=5507):
    ...
    command = [sys.executable, "-m", "nav.scripts.agent.run_benchmark_cell",
...
def main():
    ...
    raise SystemExit(subprocess.call(command, cwd=Path(__file__).resolve().parents[3], env=env))
