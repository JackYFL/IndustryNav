...
dispatches one ``nav.scripts.agent.run_benchmark_cell`` invocation per cell into a
...
# nav/scripts/agent/run_benchmark_grid.py -> repo root is three parents up.
...
def run_cell(args_dict: dict) -> dict:
    ...
    cmd += [
        python_bin, "-m", "nav.scripts.agent.run_benchmark_cell",
        "--baseline", "llm",
        "--file_name", file_name,
...
# ---------------------------------------------------------------------------
def parse_args():
    p = argparse.ArgumentParser(description="Grid runner for nav.scripts.agent.run_benchmark_cell.")
