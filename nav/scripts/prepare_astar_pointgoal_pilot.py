...
def build_manifest(
    ...
    if pairing not in {"cross", "canonical"}:
        raise ValueError("pairing must be 'cross' or 'canonical'")
    pairs = PAIR_INDICES if pairing == "cross" else tuple((i, i) for i in range(4))
    records = []
    for scene in SCENE_CODES:
        ...
        for seed in range(seeds):
            for pair_index, (start_index, target_index) in enumerate(pairs, 1):
                ...
                prefix = "pg" if pairing == "cross" else "canonical"
                ...
                records.append(
                    {
                        ...
                        # Deterministic near-80/20 moving/static mixture across
                        # pairs and seeds, including the four-pair canonical set.
                        "dynamic_objects": (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pairing", choices=("cross", "canonical"), default="cross")
...
def main() -> None:
    ...
    spec = {
        ...
        "canonical_benchmark_pairs_included": args.pairing == "canonical",
